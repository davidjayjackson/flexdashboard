---
title: "AAVSO Validation Dashboard"
output: 
...
#
alf_ori <- mydata %>% filter(Year ==2019 & band=="Vis.") %>% select(star_name,Ymd,Year,Month,band,magnitude)
```
...
alf_ori %>% plot_ly() %>% add_boxplot(x=~Month,y=~magnitude) %>%
  layout(title = ": 2010 and Beyond") %>% layout(yaxis = list(autorange = "reversed"))
```
